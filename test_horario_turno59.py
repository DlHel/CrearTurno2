...
def main():
    try:
        ...
        # Obtener el turno ID=59 de la base de datos
        turno_original = obtener_turno_por_id(connection, 59)
        
        if not turno_original:
            print("No se pudo continuar sin el turno original")
            connection.close()
            return
        
        # Crear un turno de prueba basado en el turno original
        turno_prueba = Turno()
        turno_prueba.id_turno = None  # No asignamos ID para buscar por configuración
        turno_prueba.nombre = f"Turno de prueba basado en ID={turno_original.id_turno}"
        
        # Copiar los detalles del turno original
        for detalle_original in turno_original.detalles:
            detalle_prueba = TurnoDetalleDiario(
            ...
            turno_prueba.detalles.append(detalle_prueba)
        
        print(f"\nTurno de prueba creado: ID={turno_prueba.id_turno}, Nombre={turno_prueba.nombre}")
