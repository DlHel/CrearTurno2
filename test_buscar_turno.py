...
def buscar_turnos_similares(connection, turno):
    ...
    if turno.id_turno:
        print(f"Verificando si existe un turno con ID={turno.id_turno}")
        query_id = """
        ...
        cursor.execute(query_id, {"id_turno": turno.id_turno})
        results_id = cursor.fetchall()
        cursor.close()
        
        if results_id:
            print(f"Se encontró un turno con ID={turno.id_turno}")
            print("Detalles del turno encontrado:")
            for row in results_id:
                ...
                print(f"  - {jornada}: {hora_ingreso} - Duración: {duracion} min - Salida: {hora_salida}")
            
            return [(turno.id_turno, "Turno existente", [])]
...
def main():
    try:
        ...
        # Primero, obtener los detalles exactos del turno ID=58
        print("Obteniendo detalles del turno ID=58...")
        query_turno = """
        SELECT t.ID_TURNO, t.NOMBRE, t.VIGENCIA, t.FRECUENCIA,
               tdd.ID_TURNO_DETALLE_DIARIO, tdd.JORNADA, tdd.HORA_INGRESO, tdd.DURACION
        FROM ASISTENCIAS.TURNO t
        ...
        results_turno = cursor.fetchall()
        cursor.close()
        
        if not results_turno:
            print("No se encontró el turno ID=58")
            return
        
        print(f"Se encontraron {len(results_turno)} detalles para el turno ID=58")
        
        # Crear un turno de prueba basado en el turno ID=58
        turno = Turno()
        turno.id_turno = None  # No asignamos ID para buscar por configuración
        turno.nombre = "Turno de prueba"
        
        # Agregar todos los detalles del turno ID=58
        for row in results_turno:
            jornada = row[5]
            hora_ingreso = row[6]
            duracion = row[7]
